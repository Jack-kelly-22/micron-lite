...
class Interface:
    def __init__(self, async_loop):
        ...
        button = tk.Button(
            self.root, text="Select Frames", command=self.browseFiles
        ).place(x=230, y=20)
        # output dirrectory button
        # button = tk.Button(self.root, text="Select Folder", command=self.browseOutput).place(x=170, y=350)
        button = tk.Button(
            self.root, text="Add Job To Queue", command=self.execute_outputs
        ).place(x=250, y=470)
        ...
        contants_header = tk.Label(self.root, text="Options", font=font_1).place(
            x=x_const + 140, y=y_const
        )
        y_const += 60
        fiber_type = tk.OptionMenu(
            self.root,
            self.options["constants"]["fiber_type"],
            "Dark",
            "Light",
            "Circles",
        )
        ...
        y_const += y_space
        ignore_text = tk.Label(self.root, text="Size to ignore(microns)").place(
            x=x_const, y=y_const
        )
        ...
        y_const += y_space
        name_text = tk.Label(self.root, text="Job name", font=font_2).place(
            x=x_const, y=y_const
        )
    ...
    def create_warning_frame(self):
        ...
        # max circle diameter
        tk.Label(self.root, text="Max diameter(microns)", font=font_2).place(
            x=20, y=430
        )
    ...
    def viewFolder(self, cwd):
        ...
        for folder in folder_list:
            ...
            checkbutton = tk.Checkbutton(
                self.frame.scrollable_frame, text=folder, var=chkValue, width=20
            )
            checkbutton.pack()
    ...
    def execute_outputs(self):
        ...
        threading.Thread(
            target=_asyncio_thread, args=(self.async_loop, self.options)
        ).start()
